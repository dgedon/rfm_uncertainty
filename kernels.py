...
def euclidean_distances(samples, centers, squared=True):
    ...
    if samples is centers:
        centers_norm2 = samples_norm2.clone()
    else:
...
def rbf_M(samples, centers, M, bandwidth):
    assert bandwidth > 0
    kernel_mat = euclidean_distances_M(samples, centers, M, squared=True)
    del samples, centers
    kernel_mat.clamp_(min=0)
    kernel_mat.mul_(-1. / (2 * bandwidth**2))
    kernel_mat.exp_()
    return kernel_mat
...
def laplacian_M_grad1(samples, centers, M, bandwidth):
    ...
    kernel_mat.div_(dist)
    kernel_mat[kernel_mat == float("Inf")] = 0.
    return kernel_mat.div_(-bandwidth)
...
def gaussian_M_grad1(samples, centers, M, bandwidth):
    ...
    return -kernel_mat.div_(bandwidth**2)
